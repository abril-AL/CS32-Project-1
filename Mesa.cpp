...
#include "Gark.h" //need ? Yes
#include "Player.h" //need ? Yes
//#include "globals.h"
//#include "History.h"
...
Mesa::Mesa(int nRows, int nCols)
    : m_history(nRows, nCols) , m_rows(nRows), m_cols(nCols) ,
      m_player(nullptr) , m_nGarks(0)

// -out of order, g++ complains-
//    : m_rows(nRows), m_cols(nCols), m_player(nullptr), 
//    m_nGarks(0) , m_history(nRows,nCols)
{
...
{ //is moving to r,c possible ? return 0 or 1 & change r and c
    switch (dir)
    {
...
{
    //returns a reference to m_history
    return m_history;
}